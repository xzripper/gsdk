...
public class Vignette2D {
    ...
    public Vignette2D(float vRadius_, float vBlur_, Vector3Df vCol_, float vPosX_, float vPosY_) {
        if(!inRange(vBlur_, 0.0f, 2.0f)) {
            vBlur_ = (float) clamp(0.0, 2.0, vBlur_);

            warning("Invalid vignette blur value: clamped it between 0.0 and 2.0.");
        }
    ...
    public void setVBlur(float blur) {
        ...
        if(!inRange(vBlur, 0.0f, 2.0f)) {
            vBlur = (float) clamp(0.0, 2.0, vBlur);
