...
class Perlin {
    ...
    protected double fractalNoise(double x, double y, double z, int octaves, double persistence) {
        double total = 0;
        double frequency = 1;
        double amplitude = 1;
        double maxValue = 0;

        for(int i = 0; i < octaves; i++) {total += noise(x * frequency, y * frequency, z * frequency) * amplitude;maxValue += amplitude;amplitude *= persistence;frequency *= 2;}
...
public class NoiseGenerator {
     ...
     */
    public static double genPerlinFractalNoise(int seed, double x, double y, double z, int octaves, float persistence) {
        return new Perlin(seed).fractalNoise(x, y, z, octaves, persistence);
    }
     ...
     * @param height Height.
     */
    public static Raylib.Image genPerlinFractalNoiseImg(int seed, int scale, int octaves, float persistence, int width, int height) {
        ByteBuffer buffer = ByteBuffer.allocateDirect(width * height * 4).order(nativeOrder());
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                double value = perlin.fractalNoise(scaleNoiseCoord((double) x / width, scale), scaleNoiseCoord((double) y / height, scale), 0, octaves, persistence);
     ...
     * @param height Height.
     */
    public static Raylib.Image genSimplexNoiseImg(int seed, int scale, int width, int height) {
        ByteBuffer buffer = ByteBuffer.allocateDirect(width * height * 4).order(nativeOrder());
